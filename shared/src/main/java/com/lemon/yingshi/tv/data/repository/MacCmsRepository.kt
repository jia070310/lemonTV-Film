...
class MacCmsRepository @Inject constructor(
    ...
    private var cachedPlayerParseEndpoints: List<String> = emptyList()
    ...
    suspend fun fetchRecommendedVods(
        ...
        /** 手机推荐页：仅快速探测，无数据立即返回，不 deep scan */
        ...
        val all = if (homePreviewLimit != null || quickPreview) {
            ...
            when {
                cached != null && (!effectiveFullScan || cachedRecommendedFullScanComplete) -> cached
                else -> fetchAllRecommendedLevel9(level = level, fullScan = effectiveFullScan)
            }
        }
    ...
    /** 内存中是否已有该等级的推荐列表（含首页快速预览产生的部分缓存） */
    suspend fun hasRecommendedCache(level: Int = 9): Boolean =
        !peekRecommendedCache(level).isNullOrEmpty()

    /** 完整深扫是否已完成，推荐 Tab 可直接命中缓存 */
    suspend fun hasWarmRecommendedCache(level: Int = 9): Boolean {
        if (!hasRecommendedCache(level)) return false
        return cachedRecommendedFullScanComplete && !cachedRecommendedItems.isNullOrEmpty()
    }

    suspend fun peekRecommendedCache(level: Int = 9): List<MacCmsVodItem>? {
        val baseUrl = getServerUrl()
        if (cachedRecommendedLevel == level && cachedRecommendedUrl == baseUrl) {
            return cachedRecommendedItems
        }
        return null
    }

    /** 推荐 Tab：一次返回全量列表，避免分页循环 */
    suspend fun fetchAllRecommendedList(
        level: Int = 9,
        forceRefresh: Boolean = false
    ): MacCmsCategoryFetchResult {
        if (forceRefresh) invalidateRecommendedCache()
        val needFullScan = forceRefresh || !hasWarmRecommendedCache(level)
        val all = fetchAllRecommendedLevel9(level = level, fullScan = needFullScan)
        return MacCmsCategoryFetchResult(items = all, total = all.size)
    }

    /** 首页预览加载后后台预热，进入推荐 Tab 时可直接读缓存 */
    suspend fun preloadRecommendedFullList(level: Int = 9) {
        if (hasWarmRecommendedCache(level)) return
        fetchAllRecommendedLevel9(level = level, fullScan = true)
    }
    ...
    private suspend fun fetchAllRecommendedLevel9(level: Int, fullScan: Boolean): List<MacCmsVodItem> =
        recommendedFetchMutex.withLock {
            fetchAllRecommendedLevel9Locked(level, fullScan)
        }

    private suspend fun fetchAllRecommendedLevel9Locked(level: Int, fullScan: Boolean): List<MacCmsVodItem> {
        val baseUrl = getServerUrl()
        if (
            cachedRecommendedItems != null &&
        ...
        // 并行快速预览不得覆盖已完成的完整扫描结果
        if (cachedRecommendedFullScanComplete && !fullScan) {
            return cachedRecommendedItems.orEmpty()
        }
    ...
    private fun cacheRecommendedResult(
        ...
        cachedRecommendedItems = items
        cachedRecommendedLevel = level
        cachedRecommendedUrl = baseUrl
        cachedRecommendedFullScanComplete = fullScanComplete
    }
    ...
    private suspend fun fetchRecommendedFromShowPage(
        ...
        for (match in SHOW_PAGE_VOD_ID_PATTERN.findAll(html)) {
            if (vodIds.size >= SHOW_PAGE_MAX_VOD_IDS) break
    ...
    private suspend fun fetchRecommendedLevelBrokenScan(
        ...
        if (fullScan) {
            scanProvidePagesForLevel(
                ...
                maxPages = RECOMMENDED_DESC_SCAN_MAX_PAGES,
                idlePageLimit = RECOMMENDED_DESC_IDLE_PAGE_LIMIT,
                stopWhenCount = null
            )
        } else {
            scanProvidePagesForLevel(
                ...
                collected = collected,
                firstPage = if (collected.isEmpty()) firstPage else null,
                maxPages = RECOMMENDED_FAST_SCAN_MAX_PAGES,
                idlePageLimit = RECOMMENDED_FAST_IDLE_PAGE_LIMIT,
                stopWhenCount = RECOMMENDED_FAST_STOP_COUNT
            )
        }
    ...
    private suspend fun absorbLevelMatchedRows(
        ...
        val needDetail = mutableListOf<MacCmsVodItem>()
        for (row in rows) {
            when (row.vodLevel) {
                level -> collected[row.vodId] = resolveItemAssets(baseUrl, row)
                null, 0 -> needDetail.add(row)
                else -> Unit
            }
        }
        ...
        val detailMap = fetchVodRowsByDetailIds(
            baseUrl = baseUrl,
            ids = needDetail.map { it.vodId },
            chunkSize = RECOMMENDED_DETAIL_CHUNK_SIZE
        )
        for (row in needDetail) {
    ...
    companion object {
        ...
        private const val SHOW_PAGE_MAX_VOD_IDS = 500
    ...
    suspend fun advanceFilterCatalog(
        ...
        fun finish(exhaustedAll: Boolean): FilterCatalogResult {
            ...
            val poolCap = targetSortedCount
                ...
                .coerceAtMost(MacCmsFilterSupport.FILTER_CATALOG_POOL_CAP)
            return FilterCatalogResult(
                continuation = state,
                sorted = sorted.take(poolCap.coerceAtMost(sorted.size)),
    ...
    suspend fun getPlayerParseEndpoints(forceRefresh: Boolean = false): List<String> {
        val baseUrl = macCmsPreferences.normalizeBaseUrl(getServerUrl())
        if (baseUrl.isBlank()) return emptyList()
        if (!forceRefresh &&
            cachedPlayerParseEndpoints.isNotEmpty() &&
            cachedPlayerConfigBaseUrl == baseUrl
        ) {
            return cachedPlayerParseEndpoints
        }
        refreshPlayerShowNames(baseUrl)
        return cachedPlayerParseEndpoints
    }
    ...
    private suspend fun refreshPlayerShowNames(baseUrl: String): Map<String, String> {
        return withContext(Dispatchers.IO) {
            ...
            for (url in candidateUrls) {
                val configs = fetchPlayerConfigsFromUrl(url)
                if (configs.isNotEmpty()) {
                    cachedPlayerShowNames = configs.mapValues { (_, config) -> config.show }
                        .filterValues { it.isNotBlank() }
                    cachedPlayerParseEndpoints = configs.values
                        .filter { it.ps && it.parse.isNotBlank() }
                        .map { it.parse }
                        .distinct()
                    cachedPlayerConfigBaseUrl = normalizedBase
                    return@withContext cachedPlayerShowNames
                }
            }
            emptyMap()
        }
    ...
    private fun fetchPlayerConfigsFromUrl(url: String): Map<String, com.lemon.yingshi.tv.data.remote.parser.MacCmsPlayerLineConfig> {
        return runCatching {
            ...
            okHttpClient.newCall(request).execute().use { response ->
                ...
                MacCmsPlayerConfigParser.parsePlayerConfigs(response.body?.string().orEmpty())
            }
        }.getOrDefault(emptyMap())
    }

    private fun fetchPlayerShowNamesFromUrl(url: String): Map<String, String> =
        fetchPlayerConfigsFromUrl(url).mapValues { (_, config) -> config.show }.filterValues { it.isNotBlank() }
