...
data class MacCmsPlayerLineConfig(
    val show: String = "",
    val parse: String = "",
...
object MacCmsPlayerConfigParser {
    ...
    fun parsePlayerShowNames(jsContent: String): Map<String, String> =
        parsePlayerConfigs(jsContent).mapValues { (_, config) -> config.show }

    fun parsePlayerConfigs(jsContent: String): Map<String, MacCmsPlayerLineConfig> {
        ...
        return runCatching {
            ...
            raw.mapValues { (_, info) ->
                MacCmsPlayerLineConfig(
                    show = info["show"]?.trim().orEmpty(),
                    parse = info["parse"]?.trim().orEmpty(),
                    ps = info["ps"] == "1"
                )
            }
